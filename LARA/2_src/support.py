...
import pandas as pd
import evall
train_data = np.array(pd.read_csv(r'1_data/train_data.csv',usecols=['userId','itemId','gerne']))

neg_data = np.array(pd.read_csv(r'1_data/neg_data.csv',usecols=['userId','itemId','gerne']))

test_data =pd.read_csv(r'1_data/test_data.csv',usecols=['itemId','gerne'])
test_data.drop_duplicates( keep='first', inplace=True) 
test_data= np.array(test_data)
user_emb_matrix = np.array(pd.read_csv(r'3_util/user_emb.csv',header=None))
user_attribute_matrix = np.array(pd.read_csv(r'3_util/user_attribute.csv',header=None))
ui_matrix = np.array(pd.read_csv(r'3_util/ui_matrix.csv',header=None))

test_item = np.array(pd.read_csv('1_data/test_item.csv',header =None).astype(np.int32)   )
test_attribute = np.array( pd.read_csv( '1_data/test_attribute.csv',header =None).astype(np.int32) )
...
train = np.array(pd.read_csv('1_data/train_data2.csv',header =None))
def shuffle():
...
neg = np.array(pd.read_csv('1_data/neg_data.csv',header =None))
def shuffle2():
...
def construt_negativedata():
    ...
    print('  this is construct _negative 1_data ')
    for i in neg_data:
        i[2]=i[2][1:-1]
...
#        print(li)
    ...
    neg.to_csv('1_data/neg_data.csv',header=None,index = 0)
...
#        print(li)
    ...
    train.to_csv('1_data/train_data2.csv',header=None,index = 0)
...
def construt_testdata():  
    ...
    item.to_csv('1_data/test_item.csv',header=None,index = 0)
    attribute=pd.DataFrame(attribute )
    attribute.to_csv('1_data/test_attribute.csv',header=None,index = 0)
